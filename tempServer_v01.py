...
import glob
import os
import time  # Import for measuring elapsed time
...
# Function to generate HTML content with a gauge and timing info
def generate_html():
    ...
    for sensor_id in sensors:
        ...
        location = sensor_locations.get(sensor_id, "Unknown Location")
        if temp_c is None:
            temp_display = "Error"
            temp_f = 0
        else:
    ...
    print(f"Elapsed Time for Sensor Readings: {elapsed_time:.2f} seconds")

    # Full HTML with timing info
    html = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
        ...
        <p>Server Elapsed Time: {elapsed_time:.2f} seconds</p>
        {sensor_html}
        <script>
